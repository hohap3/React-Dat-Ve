...
function chairReducer(state = initialState, action) {
  ...
  switch (type) {
    case `chair/${SELECT_CHAIR}`: {
      ...
      // Nếu mà số ghế đã đặt rồi thì ta ko làm gì
      if (payload.daDat) return;

      const index = newChairSelectedList.findIndex(
        (chair) => chair.soGhe === payload.soGhe
      );
      // Xảy ra khi người dùng click
      // Nếu người dùng click vào mà ghế đang chọn đã có trong list thì ta sẽ remove nó đi
      // Nếu ghế đang chọn mà chưa có trong list thì ta thêm mới vào
